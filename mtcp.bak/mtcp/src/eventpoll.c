...
{
	// pxw
//	FILE *fp = fopen ("/home/dcslab/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/mtcp_epoll_wait", "a+");
...
{
//	FILE *fp = fopen ("/home/dcslab/pxw/glusterfs-3.9.1/rpc/rpc-transport/dpdk/AddEpollEvent", "a+");
	...
	//fprintf (fp, "sockid = %d, events = %d, eq->events.ev.data.sockid = %d, start = %d, end = %d, num = %d\n",
	//			socket->id, event, socket->ep_data.sockid, eq->start, eq->end, eq->num_events);
	//fflush (fp);
	//fclose (fp);
