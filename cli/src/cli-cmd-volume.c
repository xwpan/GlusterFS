...
cli_print_detailed_status (cli_volume_status_t *status)
{
        ...
        if (status->online) {
				...
				cli_out ("%-20s : %-20d", "DPDK Port", status->dpdk_port);
        } else {
                cli_out ("%-20s : %-20s", "TCP Port", "N/A");
                cli_out ("%-20s : %-20s", "RDMA Port", "N/A");
				cli_out ("%-20s : %-20s", "DPDK Port", "N/A");
        }
...
cli_print_brick_status (cli_volume_status_t *status)
{
        ...
        while (bricklen > 0) {
                if (bricklen > fieldlen) {
                        ...
                        while (num_spaces-- != 0)
                                printf (" ");
                        if (status->port || status->rdma_port || status->dpdk_port) {
                                if (status->online)
                                        cli_out ("%-10d%-11d%-12d%-8c%-5s",
                                                 status->port,
                                                 status->rdma_port,
												 status->dpdk_port,
                                                 status->online?'Y':'N',
...
struct cli_cmd volume_cmds[] = {
        ...
        { "volume create <NEW-VOLNAME> [stripe <COUNT>] "
          ...
          "[transport <tcp|rdma|tcp,rdma|dpdk|tcp,dpdk>] <NEW-BRICK>"
