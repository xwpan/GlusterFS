...
xml_end:
		...
		// pxw 
		// add options related to DPDK part in cli 
        if ((cmd & GF_CLI_STATUS_DETAIL) == 0) {
                cli_out ("%-*s %s  %s  %s  %s  %s", CLI_VOL_STATUS_BRICK_LEN,
                         "Gluster process", "TCP Port", "RDMA Port", "DPDK Port", 
                         "Online", "Pid");
                cli_print_line (CLI_BRICK_STATUS_LINE_LEN);
        }
        if (type == GF_CLUSTER_TYPE_TIER) {
        ...
        for (i = 0; i <= index_max; i++) {
                ...
                if (!strcmp (hostname, "NFS Server") ||
					...
					// pxw
					// output DPDK port info
					memset (key, 0, sizeof(key));
					snprintf (key, sizeof (key), "brick%d.dpdk_port", i);
					ret = dict_get_int32 (dict, key, &(status.dpdk_port));
					if (ret)
						continue;
                }
