...
glusterfs_listener_init (glusterfs_ctx_t *ctx)
{
        ...
        ret = rpcsvc_transport_unix_options_build (&options,
                                                   cmd_args->sock_file); //options用初始参数赋值，默认socket
       if (ret)
                goto out;
        ...
        ret = rpcsvc_create_listeners (rpc, options, "glusterfsd"); //获得trans type 创建监听;glusterd.c的init（）中调用同样的函数，创建glusterd类型的listener 
        if (ret < 1) {
                ret = -1;
                goto out;
        }

        ret = rpcsvc_program_register (rpc, &glusterfs_mop_prog);//注册监听程序
        if (ret) {
                goto out;
        }
...
glusterfs_mgmt_init (glusterfs_ctx_t *ctx)
{
        ...
        if (cmd_args->volfile_server_port)
                port = cmd_args->volfile_server_port; //服务端daemon的监听端口,arg 24007

		// pxw
		gf_log ("pxw", GF_LOG_WARNING, "mgmt: cmd_args->volfile_server_port = %d", port);

        host = cmd_args->volfile_server; //daemon的ip地址 192.168.0.11

        if (cmd_args->volfile_server_transport &&
            !strcmp (cmd_args->volfile_server_transport, "unix")) {
                ret = rpc_transport_unix_options_build (&options, host, 0);
				gf_log ("pxw", GF_LOG_WARNING, "glusterfs mgmt is a unix rpc clnt");
        } else {
                ret = rpc_transport_inet_options_build (&options, host, port);
				gf_log ("pxw", GF_LOG_WARNING, "glusterfs mgmt is a inet rpc clnt, volfile server host = %s:%d", host, port);
        }
        if (ret)
                goto out;
        ...
        rpc = rpc_clnt_new (options, THIS, THIS->name, 8);//创建rpc一个rpc
		//rpc结构体、内存池、rpc->conn->trans->init/fini
        if (!rpc) {
                ret = -1;
        ...
        ret = rpc_clnt_register_notify (rpc, mgmt_rpc_notify, THIS);//注册rpc通知调用函数mgmt_rpc_notify
        if (ret) {
        ...
        ret = rpcclnt_cbk_program_register (rpc, &mgmt_cbk_prog, THIS);//注册rpc的回调处理函数结构mgmt_cbk_prog
		if (ret) {
                gf_log (THIS->name, GF_LOG_WARNING,
        ...
        ctx->mgmt = rpc; //rpc管理->ctx的mgmt

        ret = rpc_clnt_start (rpc); //调用rpc的conn启动trans,具体调用socket或者rdma的connect
out:
        return ret;
}
