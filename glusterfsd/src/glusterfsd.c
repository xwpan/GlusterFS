...
int
create_fuse_mount (glusterfs_ctx_t *ctx) //只有client执行具体内容
{
        int              ret = 0;
        cmd_args_t      *cmd_args = NULL;
        ...
        if (ctx->process_mode != GF_CLIENT_PROCESS) {
                gf_msg ("glusterfsd", GF_LOG_ERROR, 0, glusterfsd_msg_7);  //只有client process执行
                return -1;
        }

        master = GF_CALLOC (1, sizeof (*master),   //1,创建一个master结构体
                            gfd_mt_xlator_t);    
        if (!master)
                goto err;
        ...
        if (xlator_set_type (master, "mount/fuse") == -1) {   //设置master类型，动态载入fuse库，并把函数地址赋给master的函数指针等
        ...
        if (cmd_args->fuse_mountopts) {
                ret = dict_set_static_ptr (master->options, ZR_FUSE_MOUNTOPTS,
                                           cmd_args->fuse_mountopts); //设置字典属性
                if (ret < 0) {
                        gf_msg ("glusterfsd", GF_LOG_ERROR, 0, glusterfsd_msg_4,
                                ZR_FUSE_MOUNTOPTS);
                        goto err;
        ...
        ret = xlator_init (master); //调用该xlator的init()函数
        if (ret) {
        ...
        ctx->master = master; //把fuse xlator作为整个ctx的master
...
gf_get_process_mode (char *exec_name)
{
        ...
        } else if (!strncmp (base, "glusterd", 8)) { //启动/init.d/glusterd start
...
glusterfs_ctx_defaults_init (glusterfs_ctx_t *ctx)
{
        ...
        ctx->process_uuid = generate_glusterfs_ctx_id (); //生成服务端进程uuid
        if (!ctx->process_uuid) {
                gf_msg ("", GF_LOG_CRITICAL, 0, glusterfsd_msg_13);
                goto out;
        }

        ctx->page_size  = 128 * GF_UNIT_KB; //页大小

        ctx->iobuf_pool = iobuf_pool_new (); //io缓存池
        if (!ctx->iobuf_pool) {
                gf_msg ("", GF_LOG_CRITICAL, 0, glusterfsd_msg_14, "iobuf");
                goto out;
        }

        ctx->event_pool = event_pool_new (DEFAULT_EVENT_POOL_SIZE, //event池, 默认启动epoll，默认事件线程为1,大小16k个事件epfd
        ...
        ctx->pool = GF_CALLOC (1, sizeof (call_pool_t), gfd_mt_call_pool_t); //？池大小，细分
        if (!ctx->pool) {
        ...
        ctx->pool->frame_mem_pool = mem_pool_new (call_frame_t, 4096); //（1）frame内存池大小
        if (!ctx->pool->frame_mem_pool) {
                gf_msg ("", GF_LOG_CRITICAL, 0, glusterfsd_msg_14, "frame");
                goto out;
        }
        /* stack_mem_pool size 256 * 1024 */
        ctx->pool->stack_mem_pool = mem_pool_new (call_stack_t, 1024); //（2）栈内存池大小
        if (!ctx->pool->stack_mem_pool) {
                gf_msg ("", GF_LOG_CRITICAL, 0, glusterfsd_msg_14, "stack");
                goto out;
        }

        ctx->stub_mem_pool = mem_pool_new (call_stub_t, 1024); //（3）stub内存池
        if (!ctx->stub_mem_pool) {
                gf_msg ("", GF_LOG_CRITICAL, 0, glusterfsd_msg_14, "stub");
                goto out;
        }

        ctx->dict_pool = mem_pool_new (dict_t, GF_MEMPOOL_COUNT_OF_DICT_T); //（4）字典池？
        if (!ctx->dict_pool)
                goto out;

        ctx->dict_pair_pool = mem_pool_new (data_pair_t,
                                            GF_MEMPOOL_COUNT_OF_DATA_PAIR_T); //（5）字典对池？
        if (!ctx->dict_pair_pool)
                goto out;

        ctx->dict_data_pool = mem_pool_new (data_t, GF_MEMPOOL_COUNT_OF_DATA_T);//（6）字典数据池
        if (!ctx->dict_data_pool)
                goto out;

        ctx->logbuf_pool = mem_pool_new (log_buf_t,
                                         GF_MEMPOOL_COUNT_OF_LRU_BUF_T); //（7）日志缓存池
        if (!ctx->logbuf_pool)
                goto out;
        ...
        cmd_args = &ctx->cmd_args; //命令参数

        /* parsing command line arguments *///填充cmd_args参数
...
parse_cmdline (int argc, char *argv[], glusterfs_ctx_t *ctx)
{
        ...
        process_mode = gf_get_process_mode (argv[0]);//获得当前glusterfsd进程类型（server、glusterd、client）
        ...
        if ((cmd_args->volfile_server == NULL) //赋予volume配置文件（daemon时执行）,server不执行
...
glusterfs_process_volfp (glusterfs_ctx_t *ctx, FILE *fp)
{
        ...
        graph = glusterfs_graph_construct (fp);//把获得的volume文件制成graph
        if (!graph) {
                gf_msg ("", GF_LOG_ERROR, 0, glusterfsd_msg_26);
                goto out;
        }

        for (trav = graph->first; trav; trav = trav->next) { //如果有mount/fuse在xlator中则报错退出
        ...
        ret = glusterfs_graph_activate (graph, ctx); //初始化每个xlator并启动每个xlator的init(),其中每个brick都要单独启动各自的进程，重新执行这个glusterfsd   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        if (ret) {
                goto out;
        }

        gf_log_dump_graph (fp, graph); //在日志中输出graph
...
int
glusterfs_volumes_init (glusterfs_ctx_t *ctx)//利用rpc的socket库与glusterfs的daemon建立连接，获取卷信息文件，初始化client端的translator并初始化、注册事件处理函数
{
        FILE               *fp = NULL;
        cmd_args_t         *cmd_args = NULL;
        int                 ret = 0;
		...
		// 1.如果daemon赋予sock参数，server 利用创建一个socket listener（注释也可）
        if (cmd_args->sock_file) {
                ret = glusterfs_listener_init (ctx);
                if (ret)
                        goto out;
         }
		// 2.server创建rpc clnt同daemon建立连接，获取brick配置
        if (cmd_args->volfile_server) { 
                ret = glusterfs_mgmt_init (ctx);
				//设置字典optiond烦人host和port等信息，即连接本host的daemon的监听端口
				//创建一个rpc并初始化两个内存池、初始胡rpc变量conn，包括conn中的trans函数指针等变量、调用其init（）、注册conn->trans的通知调用函数为rpc_clnt_notify、rpc的通知调用函数mgmt_rpc+notify、rpc的回调处理函数mgmt_cbk_prog，
				//调用rpc的conn启动trans：调用trans的变量ops的函数指针connct连接daemon监听端口、this->ops->connect（this，port）、注册事件句柄和事件处理函数，epoll监听句柄
                /* return, do not emancipate() yet */
                return ret;
        }

		//daemon只执行这句以下。。
        fp = get_volfp (ctx);//读ctx的volume（server/client/..）
        ...
        ret = glusterfs_process_volfp (ctx, fp);//打印graph,daemon进程在这里初始化每个xlator的init（），创建新的glusterfsd进程
        if (ret)
                goto out;
...
main (int argc, char *argv[])
{
	...
	THIS->ctx = ctx;
        ...
        ret = glusterfs_ctx_defaults_init (ctx);//ctx默认部分初始化
        if (ret)
                goto out;

        ret = parse_cmdline (argc, argv, ctx);//用程序启动时的cmdline参数初始化ctx
        if (ret)
                goto out;
        cmd = &ctx->cmd_args;
           ...
           command line options. */
        {
                ...
                gf_msg (argv[0], GF_LOG_INFO, 0, glusterfsd_msg_30,
                        argv[0], PACKAGE_VERSION, cmdlinestr);  //日志中打印cmdline
        ...
        ret = create_fuse_mount (ctx);//!!!!!!客户端fuse挂载; deamon不执行，返回；只有client会挂载fuse
        if (ret)
                goto out;

        ret = daemonize (ctx); //把glusterd变成守护进程在后台工作/deamon化该client端
        if (ret)
                goto out;
	...
	ctx->env = syncenv_new (0, 0, 0);
        ...
        ret = glusterfs_volumes_init (ctx);//卷volume初始化(rpc，graph)
		//对于服务端的daemon来说：
		//对每个brick再创建单独的服务，每个brick服务都有各自的监听端口，通信类型（rdma或者ocket）实现过程在glusterd xlator的init（）函数中
		//daemon自己和brick之间的通信socket？
		//
        if (ret)
                goto out;

        ret = event_dispatch (ctx->event_pool);//启动epoll，创建1个dispatch worker thread，线程启动
