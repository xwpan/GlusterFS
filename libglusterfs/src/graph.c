...
glusterfs_graph_prepare (glusterfs_graph_t *graph, glusterfs_ctx_t *ctx)
{
        ...
        ret = glusterfs_graph_settop (graph, ctx); //找到graph的top （volume name）
        if (ret) {
...
int
glusterfs_graph_activate (glusterfs_graph_t *graph, glusterfs_ctx_t *ctx)//初始化graph中的各个translator，创建socket，建立事件监听函数
{
        int ret = 0;
        xlator_t *root = NULL;
        ...
        /* XXX: perform init () */
        ret = glusterfs_graph_init (graph);//!!!!!!!!!!沿着graph 的节点往下trav，对每个节点的xlator调用init（）函数初始化,此daemon只会调用mgmt的init函数（xlators/mgmt/glusterd/src/glusterd.c,y因为配置文件只有一个mgmt/glusterd 的volume），在初始化的过程中建立/etc/glusterd/下的vols，peers等文件夹，创建监听端口，设置事件处理函数，恢复上次daemon退出的状态灯操作
        if (ret) {
