...
event_pool_new_epoll (int count, int eventthreadcount)
{
        ...
        event_pool->fd = epfd; //事件池描述符

        event_pool->count = count;//最多可以监听事件个数

        event_pool->eventthreadcount = eventthreadcount; //默认使用1个线程
                              ...
                              struct epoll_event *event)
{
	...
	LOCK (&slot->lock);
	{
		...
		handler = slot->handler;//找到那个事件的handler，
		data = slot->data;
...
pre_unlock:
	...
	ret = handler (fd, idx, data, //调用handler的函数
...
event_dispatch_epoll_worker (void *data)
{
	...
	struct event_pool  *event_pool;
        ...
        myindex = ev_data->event_index;//每个epoll_event都有一个处理实现的索引（也是线程索引）
        ...
        pthread_mutex_lock (&event_pool->mutex);
        {
                event_pool->activethreadcount++;//活跃线程数+1
        }
        pthread_mutex_unlock (&event_pool->mutex);

	for (;;) {    //死循环
                if (event_pool->eventthreadcount < myindex) { //注册的事件线程数<索引，退出线程(线程数和设定不符合)
                ...
                ret = epoll_wait (event_pool->fd, &event, 1, -1); //每次取出一个事件处理？
...
event_dispatch_epoll (struct event_pool *event_pool)
{
	int                       i = 0;
        ...
        int                       pollercount = 0;
	int                       ret = -1;
        struct event_thread_data *ev_data = NULL; //线程池中的线程数据，不是事件！

        /* Start the configured number of pollers */
        pthread_mutex_lock (&event_pool->mutex);
        {
                pollercount = event_pool->eventthreadcount; //默认为1

                /* Set to MAX if greater */
                if (pollercount > EVENT_MAX_THREADS) //最大32个, 
                ...
                for (i = 0; i < pollercount; i++) { //创建pollercount个数的event_dispatch_epoll_worker 线程
                        ...
                        ret = pthread_create (&t_id, NULL,
                                              event_dispatch_epoll_worker, // 创建worker线程（默认一个）
                                              ev_data);
                        if (!ret) {
...
int
event_reconfigure_threads_epoll (struct event_pool *event_pool, int value)  //重新设置event epoll的worker thread个数
{
        int                              i;
        int                              ret = 0;
        pthread_t                        t_id;
        ...
        pthread_mutex_lock (&event_pool->mutex);
        {
                ...
                if (event_pool_dispatched_unlocked(event_pool)
                    && (oldthreadcount < value)) {
                        ...
                        for (i = oldthreadcount; i < value; i++) {  //如果新增线程，创建新县城
                                ...
                                if (event_pool->pollers[i] == 0) {
                                        ...
                                        ev_data->event_pool = event_pool;  //事件池
                                        ev_data->event_index = i + 1;  //索引

                                        ret = pthread_create (&t_id, NULL,
                                                event_dispatch_epoll_worker, //启动线程
                                                ev_data);
                                        if (ret) {
                ...
                /* if value decreases, threads will terminate, themselves */  //如果线程变少，中止运行的线程
