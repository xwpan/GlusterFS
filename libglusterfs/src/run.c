...
runner_end_reuse (runner_t *runner)
{
        ...
        //gf_log ("pxw", GF_LOG_WARNING, "runner->chpid = %d", runner->chpid);
        if (runner->chpid > 0) {
                if (waitpid (runner->chpid, &chstat, 0) == runner->chpid) {
 		//gf_log ("pxw", GF_LOG_WARNING, "chstat = %d", chstat);
                        if (WIFEXITED(chstat)) {
...
runner_run_generic (runner_t *runner, int (*rfin)(runner_t *runner))
{
        ...
        gf_log ("pxw", GF_LOG_WARNING, "runner_start = %d", ret);
        if (ret)
                goto out;
        ret = rfin (runner);
        gf_log ("pxw", GF_LOG_WARNING, "rfin = %d", ret);
...
runner_run_nowait (runner_t *runner)
{
	...
	pid = fork ();
  gf_log ("pxw", GF_LOG_WARNING, "CHILD PID = %d", pid);
	if (!pid) {
		setsid ();
