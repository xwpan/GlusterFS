...
#define MAX_FLOW_NUM  (10000) //10000

#define RCVBUF_SIZE (2*1024)
#define SNDBUF_SIZE (8*1024)

#define MAX_EVENTS (MAX_FLOW_NUM * 3)  //event pool的大小，一次最多多少并发事件
...
//		gf_log(this->name,GF_LOG_WARNING, "bind server listening socket successfully!\n");
	...
	/* listen (backlog: 4K) */
	ret = mtcp_listen(ctx->mctx, listener, 100); //listen
	if (ret < 0) {
...
create_mtcp_thread(int core) //对某个核上的app thread而言，创建对应的mTCP thread_context，启动，与该app thread交互
{
	...
	ctx->mctx = mtcp_create_context(core); //创建mtcp线程
	if (!ctx->mctx) {
	...
	//	gf_log(this->name,GF_LOG_WARNING, "server thread in while routine ......");
		...
		for (i = 0; i < nevents; i++) {
	    	...
	    	if ((events[i].data.sockid == listener) )	{ //1.新连接请求
				...
				//gf_log (this->name, GF_LOG_WARNING, "ret = %d, events[%d].events = %x",ret, i, events[i].events);
				if  (!ret && (events[i].events & MTCP_EPOLLOUT)) { //2， 客户端可读事件
					...
					//gf_log (this->name, GF_LOG_WARNING, "4-2");
			       	...
			       	if (!ret && (events[i].events & MTCP_EPOLLIN)) { //3. 客户端可写事件(finished)
				...
				if ((ret < 0) || (events[i].events & MTCP_EPOLLERR)) { //1.mtcp错误
					gf_log(this->name,GF_LOG_ERROR, "MTCP_EPOLLERRs！\n");				
