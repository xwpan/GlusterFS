                                   ...
                                   socklen_t *addr_len)
{
		...
		gf_log ("pxw", GF_LOG_WARNING, "服务端host listen_path = %s ",listen_path);
        ...
        if (listen_host_data)
        {
                ...
                if (addr->sa_family == AF_INET6) {
                        ...
                        *addr_len = sizeof(struct sockaddr_in6);
                        goto out;
                } else if (addr->sa_family == AF_INET) { //ipv4
                        ...
                        *addr_len = sizeof(struct sockaddr_in);
						...
						/* pxw */
						gf_log ("pxw", GF_LOG_WARNING, "服务端host ip地址：%s, 端口号port : %d ",listen_host, listen_port); //不明确ip，kernel在bind时确定，用默认网卡
                        goto out;
                }
        }

		/*---------  从options中获得 listen host  --------------*/
        ...
        if (address_family_data) {
                ...
                if (!strcasecmp (address_family, "inet")) {
                        *sa_family = AF_INET;
						...
						gf_log ("pxw",	GF_LOG_WARNING, "inet6");
                        *sa_family = AF_INET6;
                } else if (!strcasecmp (address_family, "inet-sdp")) {
						gf_log ("pxw",	GF_LOG_WARNING, "inet-sdp");
                        *sa_family = AF_INET_SDP;
				...
				gf_log ("pxw", GF_LOG_WARNING, "default ,using inet");
                *sa_family = AF_INET;
        }
