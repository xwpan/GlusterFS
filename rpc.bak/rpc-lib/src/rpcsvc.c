               ...
               rpc_transport_t *new_trans)
{
        ...
        rpcsvc_program_notify (listener, RPCSVC_EVENT_ACCEPT, new_trans); //rpcsvc在调用program回调，参数RPC。。。
        ret = 0;
out:
        return ret;
}
               ...
               rpc_transport_event_t event, void *data, ...)
{
        ...
        case RPC_TRANSPORT_ACCEPT:
                new_trans = data;
                ret = rpcsvc_accept (svc, trans, new_trans); //rpcsvc接收到rpc_transport使用的回调函数信息后，进行accept的后续工作
                break;
...
rpcsvc_create_listener (rpcsvc_t *svc, dict_t *options, char *name)
{
        ...
        trans = rpc_transport_load (svc->ctx, options, name); //调用init（）
        if (!trans) {
        ...
        ret = rpc_transport_listen (trans);//调用listen（）
        if (ret == -EADDRINUSE || ret == -1) {
                gf_log (GF_RPCSVC, GF_LOG_WARNING,
                        "listening on transport failed");
                goto out;
		} else if ( ret == 0) {
			gf_log (GF_RPCSVC, GF_LOG_ERROR, "rpcsvc listener create successfully");
		}

        ret = rpc_transport_register_notify (trans, rpcsvc_notify, svc); //设置trans->notify回调函数
        if (ret == -1) {
                gf_log (GF_RPCSVC, GF_LOG_WARNING, "registering notify failed");
                goto out;
        }

        listener = rpcsvc_listener_alloc (svc, trans);//往svc的listners中添加trans类型的listener
        if (listener == NULL) {
                goto out;
        }
...
rpcsvc_create_listeners (rpcsvc_t *svc, dict_t *options, char *name)
{
        ...
        data = dict_get (options, "transport-type");  //获得transport type
        if (data == NULL) {
                gf_log (GF_RPCSVC, GF_LOG_ERROR,
		...
		/* pxw */
		gf_msg ("pxw", GF_LOG_WARNING, 0, 0, "transport_type get from option字典中获得类型： %s", transport_type);
		...
		// server端volume创建一个tcp.dht-server,一个socket.dht-changelog
        while (ptr != NULL) {
                tmp = gf_strdup (ptr);
                if (tmp == NULL) {
                        goto out;
				...
				/* pxw */
				gf_msg ("pxw", GF_LOG_WARNING, 0, 0, "成功创建一个listener,transport_name:%s",transport_name);
...
rpcsvc_register_notify (rpcsvc_t *svc, rpcsvc_notify_t notify, void *mydata)
{
        ...
        pthread_mutex_lock (&svc->rpclock);
        {
                list_add_tail (&wrapper->list, &svc->notify); //把wrapper添加到svc的notify中
                svc->notify_count++;
        }
        pthread_mutex_unlock (&svc->rpclock);
...
int
rpcsvc_program_register (rpcsvc_t *svc, rpcsvc_program_t *program) //把program添加进svc->programs链表
{
        int               ret                = -1;
...
rpcsvc_transport_unix_options_build (dict_t **options, char *filepath)
{
		...
		//把socket文件地址 给fpath，作为transport.socket.listen-path
        ...
        ret = dict_set_str (dict, "transport-type", "socket"); //默认socket？
        if (ret)
                goto out;
             ...
             uint32_t poolcount)
{
        ...
        gf_log (GF_RPCSVC, GF_LOG_TRACE, "rx pool: %d", poolcount); //创建rx接收池
        ...
        ret = rpcsvc_program_register (svc, &gluster_dump_prog);//rpcsvc程序注册
        if (ret) {
                gf_log (GF_RPCSVC, GF_LOG_ERROR,
