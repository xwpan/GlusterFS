        ...
        pthread_mutex_lock (&conn->lock);
        {
                ...
                if ((conn->connected == 0) && !clnt->disabled) {
                        ...
                        (void) rpc_transport_connect (trans,
                                                      conn->config.remote_port); //调用rpc_transport_connect（初次建立连接时。。）
                        rpc_clnt_ref (clnt);
                        conn->reconnect =
...
rpc_clnt_handle_cbk (struct rpc_clnt *clnt, rpc_transport_pollin_t *msg)
{
        ...
        clnt = rpc_clnt_ref (clnt);
        ret = xdr_to_rpc_call (msgbuf, msglen, &rpcmsg, &progmsg, NULL,NULL); //rpc解编码
        if (ret == -1) {
        ...
        if (found && (procnum < program->numactors) &&
            (program->actors[procnum].actor)) {
                program->actors[procnum].actor (clnt, program->mydata,
                                                &progmsg);//调用rpc program actor（某个处理函数）
        }
                          ...
                          dict_t *options, char *name)
{
        ...
        ret = dict_get_int32 (options, "frame-timeout",   //把option中的与frame-timeout相关的值赋值给conn的成员？
                              &conn->frame_timeout);
        if (ret >= 0) {
        ...
        ret = dict_get_int32 (options, "ping-timeout",  //同上 
                              &conn->ping_timeout);
        if (ret >= 0) {
                gf_log (name, GF_LOG_DEBUG,
        ...
        trans = rpc_transport_load (ctx, options, name); //载入通信模块,这里构建rdma或者socket类型的trans
        if (!trans) {
        ...
        ret = rpc_transport_register_notify (conn->trans, rpc_clnt_notify, //注册conn->trans的通知调用函数rpc_clnt_notify
                                             conn);
        if (ret == -1) {
              ...
              uint32_t reqpool_size)
{
        ...
        rpc->ctx = ctx; //glusterfs 的 ctx
        rpc->owner = owner;

        if (!reqpool_size)
                reqpool_size = RPC_CLNT_DEFAULT_REQUEST_COUNT;

        rpc->reqpool = mem_pool_new (struct rpc_req, reqpool_size); //创建请求池
        if (rpc->reqpool == NULL) {
        ...
        rpc->saved_frames_pool = mem_pool_new (struct saved_frame,
                                               reqpool_size);  //存储的frame池, 存储rpc数据
        ...
        ret = rpc_clnt_connection_init (rpc, ctx, options, name); //初始化client链接,确定transport类型,conn->trans=rpc_transport_load
        if (ret == -1) {
                pthread_mutex_destroy (&rpc->lock);
                pthread_mutex_destroy (&rpc->notifylock);
                mem_pool_destroy (rpc->reqpool);
...
rpc_clnt_start (struct rpc_clnt *rpc)
{
         ...
         */
        rpc_clnt_ref (rpc);//rpc的ref+1
        rpc_clnt_reconnect (conn); //rdma/socket中的connect ops调用
                              ...
                              rpcclnt_cb_program_t *program, void *mydata)
{
        ...
        pthread_mutex_lock (&clnt->lock);
        {
                list_for_each_entry (tmp, &clnt->programs, program) { //client的program: 一些cbk函数
