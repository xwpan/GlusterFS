...
glusterd_store_brickinfo_write (int fd, glusterd_brickinfo_t *brickinfo)
{
		...
		//pxw
		snprintf (value, sizeof(value), "%d", brickinfo->dpdk_port);
		ret = gf_store_save_value (fd, GLUSTERD_STORE_KEY_BRICK_DPDK_PORT,
								   value);
...
glusterd_store_retrieve_bricks (glusterd_volinfo_t *volinfo)
{
        ...
        while (brick_count < volinfo->brick_count) {
                ...
                while (!ret) {
						...
						//pxw		
                        } else if (!strncmp (key, GLUSTERD_STORE_KEY_BRICK_DPDK_PORT,
                                    strlen (GLUSTERD_STORE_KEY_BRICK_DPDK_PORT))) {
                                gf_string2int (value, &brickinfo->dpdk_port);

                                if (brickinfo->dpdk_port < priv->base_port) {
                                        /* This is required to adhere to the
                                           IANA standards */
