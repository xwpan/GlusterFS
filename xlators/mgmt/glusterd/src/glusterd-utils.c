                        ...
                        glusterd_brickinfo_t *dup_brickinfo)
{
		...
		dup_brickinfo->dpdk_port = brickinfo->dpdk_port; //pxw
                                 ...
                                 gf_boolean_t wait)
{
        ...
        glusterd_set_brick_socket_filepath (volinfo, brickinfo, socketpath,//设置sock文件路径/var/run/md5.socket
                                            sizeof (socketpath));

        GLUSTERD_GET_BRICK_PIDFILE (pidfile, volinfo, brickinfo, priv);//设置pid文件
        if (gf_is_service_running (pidfile, NULL))
                goto connect;

        port = pmap_assign_port (THIS, brickinfo->port, brickinfo->path);//设置socket端口
...
retry:
		...
		//pxw
		gf_log ("pxw", GF_LOG_WARNING, "volinfo->transport_type = %d", volinfo->transport_type);
        if (volinfo->transport_type == GF_TRANSPORT_TCP || volinfo->transport_type == GF_TRANSPORT_RDMA) {
		...
		// pxw
		// set brick server transport type
		// currently on sole dpdk supported
		 if (volinfo->transport_type == GF_TRANSPORT_BOTH_TCP_DPDK || volinfo->transport_type == GF_TRANSPORT_DPDK ) {
			snprintf(dpdk_brick_path, sizeof(dpdk_brick_path), "%s.dpdk",	//dpdk brick 路径
					 brickinfo->path);
			dpdk_port = pmap_assign_port(THIS, brickinfo->dpdk_port, dpdk_brick_path);
      gf_log ("pxw", GF_LOG_WARNING, "dpdk_port = %d", dpdk_port);
			runner_argprintf(&runner, "%d", dpdk_port);
			runner_add_arg (&runner, "--xlator-option");
			runner_argprintf (&runner, "%s-server.transport.dpdk.listen-port=%d", 
							 volinfo->volname, dpdk_port);

			// let port be dpdk_port
			port = dpdk_port;
		 }
		

        runner_add_arg (&runner, "--xlator-option");
        runner_argprintf (&runner, "%s-server.listen-port=%d",  //输出
                          volinfo->volname, port); 

		gf_log ("pxw", GF_LOG_WARNING, "transport.socket.bind-address = %s", bind_address);
		...
		if (volinfo->transport_type == GF_TRANSPORT_DPDK) {
			
      runner_add_arg (&runner, "--xlator-option");
			dpdk_bind_address = "10.5.0.112";

			strcpy(brickinfo->dpdk_hostname, dpdk_bind_address);
			runner_argprintf (&runner, "%s-server.transport.dpdk.bind-address=%s", volinfo->volname, dpdk_bind_address);
        ...
        if (volinfo->transport_type == GF_TRANSPORT_RDMA)
		    ...
		    else if (volinfo->transport_type == GF_TRANSPORT_DPDK)
				        runner_argprintf (&runner, "--volfile-server-transport=dpdk");
		    else if (volinfo->transport_type == GF_TRANSPORT_BOTH_TCP_DPDK) //currently not supported
				        runner_argprintf (&runner, "--volfile-server-transport=socket,dpdk");
		gf_log ("pxw", GF_LOG_WARNING, "2222222");

        if (volinfo->memory_accounting)
                runner_add_arg (&runner, "--mem-accounting");
		gf_log ("pxw", GF_LOG_WARNING, "33332");

        runner_log (&runner, "", 0, "Starting GlusterFS");
     
		gf_log ("pxw", GF_LOG_WARNING, "44444");
	       brickinfo->port = port; 
         brickinfo->rdma_port = rdma_port;
		     brickinfo->dpdk_port = dpdk_port;


        if (wait) {
		gf_log ("pxw", GF_LOG_WARNING, "5555");
                synclock_unlock (&priv->big_lock);
                ret = runner_run (&runner);   //重新启动brick服务！！！
                synclock_lock (&priv->big_lock);

                gf_log ( "pxw", GF_LOG_WARNING, "runner_run ret = %d", ret);
                if (ret == -EADDRINUSE) {
		...
		gf_log ("pxw", GF_LOG_WARNING, "6666");
                ret = runner_run_nowait (&runner);
                gf_log ( "pxw", GF_LOG_WARNING, "runner_run_nowait ret = %d", ret);
        }

        if (ret) {
                brickinfo->port = 0;
                brickinfo->rdma_port = 0;
				brickinfo->dpdk_port = 0;
                goto out;
        }
...
int
glusterd_spawn_daemons (void *opaque) //根据上次daemon运行状态针对每一个brick启动一个brick服务
{
        glusterd_conf_t *conf = THIS->private;
        int             ret             = -1;

        synclock_lock (&conf->big_lock);
        glusterd_restart_bricks (conf); //start bricks
        glusterd_restart_gsyncds (conf); //start glusterd peer
                      ...
                      gf_boolean_t wait)
{
        ...
        ret = glusterd_volume_start_glusterfs (volinfo, brickinfo, wait);//启动brick
		gf_msg ("pxw", GF_LOG_WARNING, 0, 0, "启动brick, brickinfo->hostname = %s, brickinfo->path = %s", brickinfo->hostname, brickinfo->path);
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_BRICK_DISCONNECTED,
        ...
        cds_list_for_each_entry (volinfo, &conf->volumes, vol_list) {
                ...
                */
                ...
                if (ret == 0) {
                        ...
                        cds_list_for_each_entry (brickinfo, &volinfo->bricks,
                                                 brick_list) {
                                glusterd_brick_start (volinfo, brickinfo,  //启动brick
                                                     _gf_false);
                        }
                }
        }
                            ...
                            dict_t  *dict, int32_t count)
{
		...
		memset (key, 0, sizeof (key));
		snprintf (key, sizeof (key), "%s.dpdk_port", base_key);
		if (volinfo->transport_type == GF_TRANSPORT_DPDK) {
				ret = dict_set_int32 (dict, key, brickinfo->dpdk_port);
		} else 
				ret = dict_set_int32 (dict, key, 0);
                              ...
                              gf_transport_type type)
{
		...
		// pxw
		// dpdk and tcp share client volfile
		case GF_TRANSPORT_DPDK:
                snprintf (filepath, PATH_MAX,
                                    ...
                                    gf_transport_type type)
{
		...
		// pxw
		case GF_TRANSPORT_DPDK:
		case GF_TRANSPORT_BOTH_TCP_DPDK:
                snprintf (filepath, PATH_MAX,
                                        ...
                                        char *transport_type_str)
{
        ...
        case GF_TRANSPORT_BOTH_TCP_RDMA:
                ...
                break;
		case GF_TRANSPORT_DPDK:
				sprintf (transport_type_str, "%s", "dpdk");
				break;
		case GF_TRANSPORT_BOTH_TCP_DPDK:
				sprintf (transport_type_str, "%s", "tcp_dpdk_both");
				break;
        default:
                goto out;
