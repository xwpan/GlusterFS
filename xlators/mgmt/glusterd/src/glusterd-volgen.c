...
transport_str_to_type (char *tt)
{
		...
		else if (!strcmp ("dpdk", tt))
				type = GF_TRANSPORT_DPDK;
		else if (!strcmp ("tcp,dpdk", tt))
				type = GF_TRANSPORT_BOTH_TCP_DPDK;
        return type;
}
                    ...
                    char *transt, gf_boolean_t is_nfs)
{
        ...
        transport_type get_transport;  //从volfile中获得transport类型

        if (is_nfs == _gf_false) {
                key = "client-transport-type"; //客户
                get_transport = get_vol_transport_type;
        } else {
                key = "nfs.transport-type"; //nfs
                get_transport = get_vol_nfs_transport_type;
        }

        ret = dict_get_str (set_dict, key, &tt);
        if (ret)
                get_transport (volinfo, transt); //执行
        if (!ret)
                strcpy (transt, tt);
}
                         ...
                         dict_t *set_dict, glusterd_brickinfo_t *brickinfo)
{
		...
		// pxw
		if (dict_get (THIS->options, "transport.dpdk.bind-address")) {
				ret = xlator_set_option (xl, "transport.dpdk.bind-address",
										 brickinfo->dpdk_hostname);
				gf_log ("pxw", GF_LOG_WARNING, "DPDK_hostname = %s", brickinfo->dpdk_hostname);
				if (ret)
					return -1;
		}
                                 ...
                                 dict_t *mod_dict, void *data)
{
		...
		gf_log ("pxw", GF_LOG_WARNING, "glusterd_generate_brick_volfile path = %s", filename);
                          ...
                          glusterd_client_type_t client_type)
{
		...
		// pxw
		// if server trans type is dpdk, we don't need to change client trans type from tcp to dpdk
		// dpdk ip address should not be changed here...
	
	    if ((volinfo->transport_type != GF_TRANSPORT_DPDK) &&
		    (volinfo->transport_type != GF_TRANSPORT_BOTH_TCP_DPDK))
			enumerate_transport_reqs (volinfo->transport_type, types);
		else {
			gf_log ("pxw", GF_LOG_WARNING, "dpdk case: dont generate new client volfiles");
			ret = 0;
			return ret;
		} 
        ...
        for (i = 0; types[i]; i++) {
				...
				//pxw：  set volume file（dht.tcp-fuse.vol / dht,rdma-fuse.vol / trusted-dht.rdma/tcp-fuse/vol）
				//we simply consider dpdk sharing the tcp-fuse volfile
				//keep transport type same but change to dpdk ip address
		...
		//pxw : doesnt create balance volume when only 1 brick in a volume
		//keep transport type tcp but change to dpdk ip address
...
glusterd_create_volfiles (glusterd_volinfo_t *volinfo)
{
        ...
        ret = generate_brick_volfiles (volinfo);//修改brick file
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_VOLFILE_CREATE_FAIL,
        ...
        ret = generate_client_volfiles (volinfo, GF_CLIENT_TRUSTED); //生成client file trusted
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_VOLFILE_CREATE_FAIL,
        ...
        ret = generate_client_volfiles (volinfo, GF_CLIENT_OTHER); //other client file
        if (ret)
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_VOLFILE_CREATE_FAIL,
...
glusterd_create_volfiles_and_notify_services (glusterd_volinfo_t *volinfo)
{
        ...
        ret = glusterd_create_volfiles (volinfo); //修改volfile参数后
        if (ret)
                goto out;
