...
__gluster_pmap_signout (rpcsvc_request_t *req)
{
		...
		//pxw
		// dpdk share brick path with tcp（not sure ...）
		//else if (args.dpdk_port) { 
		//		snprintf(brick_path, PATH_MAX, "%s.dpdk", args.brick);
		//		rsp.op_ret = pmap_registry_remove (THIS, args.dpdk_port,
		//						brick_path, GF_PMAP_PORT_BRICKSERVER,
		//						req->trans);
		//}
