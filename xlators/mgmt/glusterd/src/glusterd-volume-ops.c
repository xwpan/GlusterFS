...
glusterd_op_create_volume (dict_t *dict, char **op_errstr)
{
        ...
        } else if (strcasecmp (trans_type, "dpdk") == 0) {
				volinfo->transport_type = GF_TRANSPORT_DPDK;
		} else if (!strcasecmp (trans_type, "tcp") && !strcasecmp (trans_type, "dpdk")) {
...
glusterd_op_start_volume (dict_t *dict, char **op_errstr)
{
        ...
        ret = glusterd_op_start_volume_args_get (dict, &volname, &flags);
        gf_log ("pxw", GF_LOG_WARNING, "11");
        if (ret)
                goto out;

        ret  = glusterd_volinfo_find (volname, &volinfo);
        gf_log ("pxw", GF_LOG_WARNING, "22");
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
        ...
        gf_log ("pxw", GF_LOG_WARNING, "4---5");
        ret = glusterd_start_volume (volinfo, flags, _gf_true);
        gf_log ("pxw", GF_LOG_WARNING, "55");
        if (ret)
                goto out;
