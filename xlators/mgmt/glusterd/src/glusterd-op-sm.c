...
glusterd_op_stage_set_volume (dict_t *dict, char **op_errstr)
{
        ...
        for ( count = 1; ret != 1 ; count++ ) {
                ...
                if (strcmp (key, "config.transport") == 0) {
			      ...
			      (strcasecmp (value, "dpdk") == 0) || 
			      (strcasecmp (value, "tcp,dpdk") == 0) )) {
...
glusterd_op_set_volume (dict_t *dict, char **errstr)
{
        ...
        for (count = 1; ret != -1 ; count++) {
                ...
                if (strcmp (key, "config.transport") == 0) {
                        ...
                        } else if ((strcasecmp (value, "dpdk") == 0)) { //dpdk
								volinfo->transport_type = GF_TRANSPORT_DPDK;
						} else if ((strcasecmp (value, "tcp,dpdk") == 0) ||
								   (strcasecmp (value, "dpdk,tcp") == 0)) {
...
glusterd_op_modify_op_ctx (glusterd_op_t op, void *ctx)
{
        ...
        case GD_OP_STATUS_VOLUME:
				...
				//pxw
				//not sure whether change dpdk port is needed..
