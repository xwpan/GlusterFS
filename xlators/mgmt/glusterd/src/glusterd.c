...
glusterd_init_uds_listener (xlator_t *this)
{
        ...
        ret = rpcsvc_transport_unix_options_build (&options, sockfile); //默认sokcet连接方式和client交互
        if (ret)
                goto out;
 ...
 */
int
init (xlator_t *this)    //glusterd的xlator启动,daemon只启动 mgmt/glusterd
{
        int32_t            ret                        = -1;
        rpcsvc_t          *rpc                        = NULL;
        ...
        setenv ("GLUSTERD_WORKDIR", workdir, 1);  //设置工作目录
        gf_msg (this->name, GF_LOG_INFO, 0,
                GD_MSG_CURR_WORK_DIR_INFO, "Using %s as working directory", //11111111
                workdir);
        ...
        ret = mkdir_p (GLUSTER_SHARED_STORAGE_BRICK_DIR, 0777,  //创建目录
                       _gf_true);
        if (ret) {
                gf_msg (this->name, GF_LOG_CRITICAL, 0,
        ...
        snprintf (storedir, PATH_MAX, "%s/vols", workdir);//创建/vols/
        ...
        snprintf (storedir, PATH_MAX, "%s/peers", workdir);  //远程glusterd
        ...
        ret = glusterd_rpcsvc_options_build (this->options); //创建rpc service
        if (ret)
                goto out;
        rpc = rpcsvc_init (this, this->ctx, this->options, 64); //初始化全局rpc service
        if (rpc == NULL) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
        ...
        ret = rpcsvc_register_notify (rpc, glusterd_rpcsvc_notify, this); //注册通知notify
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
        ...
        /* Enable  for the TCP listener is management encryption is
         * enabled
         */
        if (this->ctx->secure_mgmt) { //tcp消息安全，默认关闭
         ...
         */
		//只有一个（最多一对rdma和socket的listener）负责监听

        ret = dict_get_str (this->options, "transport-type", &transport_type);//从配置文件中获得option transport-type：rdma或者socket,或者rdma
        if (ret) {
                gf_msg (this->name, GF_LOG_ERROR, 0,
        ...
        total_transport = rpc_transport_count (transport_type);//一共几种transport
		...
		gf_msg ("pxw",GF_LOG_WARNING,0,0, "total_transport = %d\n",total_transport);

        ret = rpcsvc_create_listeners (rpc, this->options, this->name); //返回创建成功的listener个数
        gf_msg ("pxw", GF_LOG_WARNING,0,0,"%d listener has been created\n", ret);
        ...
        if (ret < 1) {
                ...
                ret = -1;
                goto out;
        } else if (ret < total_transport) { //rdma和socket两种，rdma初始化失败，继续用rdma初始化
                gf_msg (this->name, GF_LOG_ERROR, 0,
                        GD_MSG_RPC_LISTENER_CREATE_FAIL,
                        "creation of %d listeners failed, continuing with "
                        "succeeded transport", (total_transport - ret));//33333333333
        }

        for (i = 0; i < gd_inet_programs_count; i++) {
                ret = glusterd_program_register (this, rpc,
                                                 gd_inet_programs[i]); //把gd_inet_programs[i]添加进rpcsvc链表. rpc服务中，客户端可以调用的程序集
                if (ret) {
                        i--;
                        for (; i >= 0; i--)
                                rpcsvc_program_unregister (rpc,
         ...
         */
        uds_rpc = glusterd_init_uds_listener (this);//启动专门监听cli 命令的socket listener(重新根据参数启动，load module，init，listen，etc)  ！！！！
        if (uds_rpc == NULL) {
                ret = -1;
                goto out;
        }
        ...
        ret = dict_get_str (this->options, "run-with-valgrind", &valgrind_str); //valgrind--内存调试工具
        if (ret < 0) {
                gf_msg_debug (this->name, 0,
        ...
        if (glusterd_get_peers_count () < 2)
                glusterd_launch_synctask (glusterd_spawn_daemons, NULL); //start glusterfsd bricks and glusterd peer
        ...
        if (workers > 0 && workers != conf->workers) {
                ...
                ret = event_reconfigure_threads (this->ctx->event_pool,
                                                 workers); //重新配置dispatch worker thread个数
                if (ret)
                        goto out;
        }
