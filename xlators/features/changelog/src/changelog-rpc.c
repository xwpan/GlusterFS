...
// 把客户端发来的rpcsvc_request赋给 changelog_handle_probe，
// 创建一个新的changelog_rpc_clnt,执行rpc调用，请求changelog_rpcsvc, 因为是本地请求，使用unix domain socket模式
int
changelog_handle_probe (rpcsvc_request_t *req)
{
        ...
        changelog_probe_req   rpc_req = {0,}; //changelog_probe_req中包含sock[SOCK_PATH]
