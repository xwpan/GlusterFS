...
client_init_rpc (xlator_t *this)
{
        ...
        conf->rpc = rpc_clnt_new (this->options, this, this->name, 0); //创建各种pool包括rpc通信transport的选择
        if (!conf->rpc) {
        ...
        ret = rpcclnt_cbk_program_register (conf->rpc, &gluster_cbk_prog,  //注册回调函数cbk program
                                            this);
        if (ret) {
...
init (xlator_t *this)
{
        ...
        /* Set event threads to the configured default */ //事件池一个线程
        GF_OPTION_INIT("event-threads", conf->event_threads, int32, out);
        ret = client_check_event_threads (this, conf, STARTING_EVENT_THREADS,
                                          conf->event_threads);   //检查dispatch thread的个数，如果默认 STARTING_EVENT_THREADS < conf->event_threads，设置成后者个数，并新增/终止线程
        if (ret)
                goto out;
        ...
        */
        ...
        this->local_pool = mem_pool_new (clnt_local_t, 64); //64个？
        if (!this->local_pool) {
                ret = -1;
                gf_msg (this->name, GF_LOG_ERROR, ENOMEM, PC_MSG_NO_MEMORY,
        ...
        ret = client_init_rpc (this); //rpc初始化
out:
        if (ret)
                this->fini (this);
...
struct volume_options options[] = {
        ...
        { .key   = {"transport-type"},
          .value = {"tcp", "socket", "ib-verbs","dpdk", "unix", "ib-sdp",
