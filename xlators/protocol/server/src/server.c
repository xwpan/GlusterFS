...
reconfigure (xlator_t *this, dict_t *options)
{
		...
		//pxw
		//currently only socket support reconfigure while rdma and dpdk does not
...
init (xlator_t *this)
{
		...
		gf_msg ("pxw", GF_LOG_WARNING, 0, 0,"default/setting event_thread num = %d", conf->event_threads);
		...
		//从 this->options 中获得options中 key=transport-type
		...
		/* pxw */
		gf_msg ("pxw", GF_LOG_WARNING, 0, 0, "server创建%d个rpc_transport_t", total_transport);
...
struct volume_options options[] = {
        { .key   = {"transport-type"},
          .value = {"rpc", "rpc-over-rdma", "tcp", "socket", "dpdk", "ib-verbs",
