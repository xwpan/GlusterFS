...
gf_rdma_cm_event_handler (void *data)
{
        ...
        while (1) { //一个rdma event handler线程循环获取事件池中的事件，处理（调用rpc transport的 callback函数）
                ...
                case RDMA_CM_EVENT_ESTABLISHED:
                        gf_rdma_cm_handle_connect_init (event);//pxw
                        break;
                ...
                case RDMA_CM_EVENT_DISCONNECTED:
                        ...
                        gf_rdma_cm_handle_disconnect (this);//pxw
                        continue;
...
gf_rdma_listen (rpc_transport_t *this)
{
        ...
        ret = rdma_create_id (rdma_ctx->rdma_cm_event_channel, //rdma的事件通道？？
                              &peer->cm_id, this, RDMA_PS_TCP);
        if (ret != 0) {
                gf_msg (this->name, GF_LOG_WARNING, errno,
...
init (rpc_transport_t *this)
{
        ...
        if (gf_rdma_init (this)) { //rdma初始化，设备
                gf_msg (this->name, GF_LOG_WARNING, 0,
        ...
        pthread_mutex_lock (&rdma_ctx->lock);
        {
                if (rdma_ctx != NULL) {
                        ...
                        iobuf_pool = this->ctx->iobuf_pool; //缓冲池
