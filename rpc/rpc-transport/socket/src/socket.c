...
__socket_cached_read (rpc_transport_t *this, struct iovec *opvector, int opcount)
{
	...
	priv = this->private;
	in = &priv->incoming;
	req_len = iov_length (opvector, opcount); //请求大小
	...
	if (in->ra_read < in->ra_max)
		...
		*/
		goto out;
uncached:
	ret = __socket_ssl_readv (this, opvector, opcount); //可选择是否使用ssl
out:
	return ret;
}
              ...
              int write)
{
        ...
        while (opcount > 0) {
                if (opvector->iov_len == 0) { //开始时等于sizeof(fraghdr)
                        gf_log(this->name,GF_LOG_DEBUG,
			...
			if (priv->use_ssl) {
				...
				ret = sys_writev (sock, opvector, IOV_MIN(opcount)); //send
			}

                        if (ret == 0 || (ret == -1 && errno == EAGAIN)) { //send all
                                /* done for now */
                                break;
                        }
...
__socket_server_bind (rpc_transport_t *this)
{
		...
		//gf_log ("pxw", GF_LOG_WARNING, "服务端绑定host ip：port = %s：%d ",
    		   //((struct sockaddr_in *)&this->myinfo.sockaddr)->sin_addr.s_addr,
			   //((struct sockaddr_in *)&this->myinfo.sockaddr)->sin_port);
		//		this->myinfo.identifier, 0);
...
static int
__socket_ioq_churn_entry (rpc_transport_t *this, struct ioq *entry, int direct) //依次提交传输层的请求
{
        int               ret = -1;
	socket_private_t *priv = NULL;
	char              a_byte = 0;

        ret = __socket_writev (this, entry->pending_vector, //待发送数据已经封装好了
                               entry->pending_count,
...
static int
__socket_proto_state_machine (rpc_transport_t *this,  //处理接收到的数据？
                              rpc_transport_pollin_t **pollin)
{
        ...
        while (in->record_state != SP_STATE_COMPLETE) { //从头往下执行状态机，知道rpc记录完成服务请求为止
                ...
                case SP_STATE_READING_FRAGHDR://读取头部信息
        ...
        if (in->record_state == SP_STATE_COMPLETE) {
                ...
                __socket_reset_priv (priv); //priv重置
        }
...
socket_event_poll_in (rpc_transport_t *this)
{
	...
	ret = socket_proto_state_machine (this, &pollin); //根据rpc服务记录的状态作相应的处理；处理接收到的数据，存在this中,处理的所有结果放在pollin中

	if (pollin) {
                priv->ot_state = OT_CALLBACK;
                ret = rpc_transport_notify (this, RPC_TRANSPORT_MSG_RECEIVED,//向上调用rpcsvc_notify中注册的通知函数，数据放在pollin里面，标识是RPC_TRANSPORT_MSG_RECEIVED
                                            pollin);
                if (priv->ot_state == OT_CALLBACK) {
                        priv->ot_state = OT_RUNNING;
                }
                rpc_transport_pollin_destroy (pollin);//处理完成销毁资源
        }
                      ...
                      int poll_in, int poll_out, int poll_err)
{
	...
	int               ret = -1;
        ...
        if (priv->connected != 1) { //如果还米有建立连接先connect
        ...
        if (!ret && poll_out) {
                ret = socket_event_poll_out (this);//如果是可写事件-->发送事件
        }

        if (!ret && poll_in) {
                ret = socket_event_poll_in (this);//如果是可读事件-->读事件
        }
                             ...
                             int poll_in, int poll_out, int poll_err)
{
        ...
        pthread_mutex_lock (&priv->lock);
        {
                ...
                if (poll_in) {
                        new_sock = accept (priv->sock, SA (&new_sockaddr),  //socket接受连接，创建新的sock(连接套接字)，和监听套接字区分防止功能上的不明确
                                           &addrlen);
                        ...
                        //给new trans赋上新的rpc-transport_t属性
                        ...
                        new_trans = GF_CALLOC (1, sizeof (*new_trans),
                                               gf_common_mt_rpc_trans_t); //创建一个新的trans成员
                        if (!new_trans) {
                                sys_close (new_sock);
                                goto unlock;
                        }

                        ret = pthread_mutex_init(&new_trans->lock, NULL);  //初始化锁
                        if (ret == -1) {
                                gf_log (this->name, GF_LOG_WARNING,
                        ...
                        memcpy (&new_trans->peerinfo.sockaddr, &new_sockaddr, //初始化peerinfo属性
                                addrlen);
                        ...
                        ret = socket_init(new_trans);//初始化这个新的套接字的this->priv的属性
                        if (ret != 0) {
                                sys_close (new_sock);
                                GF_FREE (new_trans->name);
                                GF_FREE (new_trans);
                        ...
                        new_trans->ops = this->ops; //初始化new trans的其他属性（与原先的同）
				...
				cname = ssl_setup_connection(new_trans,1);
					...
					goto unlock;
                        ...
                        if (!priv->bio && !priv->own_thread) {
                                ret = __socket_nonblock (new_sock); //非阻塞
                        ...
                        pthread_mutex_lock (&new_priv->lock);
                        {
                                ...
                                if (new_priv->own_thread) {
                                        ...
                                        ret = socket_spawn(new_trans); //仅仅在使用own thread时派生新线程
                                        if (ret) {
                                                gf_log(this->name, GF_LOG_ERROR,
                                        ...
                                        new_priv->idx =
                                                event_register (ctx->event_pool,
                                                                new_sock,
                                                           socket_event_handler, //把新的连接套接字加入event pool，监听; handler函数是：socket_event_handler
                        ...
                        if (!priv->own_thread) {
                                ret = rpc_transport_notify (this,
                                        RPC_TRANSPORT_ACCEPT, new_trans); //调用通知函数,把rpc当前状态的RPC_TRANSPORT_ACCEPT参数传给rpcsvc_notify()_
                        }
                }
        }
unlock:
...
static int
socket_connect (rpc_transport_t *this, int port)  //clnt / cli 端
{
        int                            ret             = -1;
        int                            th_ret          = -1;
        int                            sock            = -1;
        ...
        pthread_mutex_lock (&priv->lock);
        {
               ...
               ign_enoent = dict_get_str_boolean (this->options,
                ...
                ret = client_bind (this, SA (&this->myinfo.sockaddr),
                                   &this->myinfo.sockaddr_len, priv->sock); //bind socket
                if (ret == -1) {
                        gf_log (this->name, GF_LOG_WARNING,
                ...
                ret = connect (priv->sock, SA (&this->peerinfo.sockaddr),
                               this->peerinfo.sockaddr_len); //与服务端建立连接
...
handler:
                ...
                if (priv->own_thread) {  //有自己的执行thread时, 在socket_init(）中设置为0
                        if (pipe(priv->pipe) < 0) {
                                gf_log(this->name,GF_LOG_ERROR,
                                "could not create pipe");
                        }
                        ...
                        priv->idx = event_register (ctx->event_pool, priv->sock,
                                                    socket_event_handler, //注册监听handler，用公用的dispatch thread
                                                    this, 1, 1);
                        if (priv->idx == -1) {
...
socket_listen (rpc_transport_t *this)
{
        ...
        pthread_mutex_lock (&priv->lock);
        {
                ...
                if (priv->backlog)
                        ...
                        ret = listen (priv->sock, 10); //该socket最多可以同时接受10个并发的连接请求
                ...
                priv->idx = event_register (ctx->event_pool, priv->sock,  //把该socket放入glusterfs的ctx的event pool,socket请求事件来临时，利用dispatch worker thread调用cbk即注册了的socket_server_event_handler
                                            socket_server_event_handler,  //这个函数接受accept连接，并且注册另一个函数进行事件的传输和处理
                                            this, 1, 0);
...
static int32_t
socket_submit_request (rpc_transport_t *this, rpc_transport_req_t *req) //客户端发送请求
{
        socket_private_t *priv = NULL;
        int               ret = -1;
	...
	char              a_byte = 'j';
        ...
        pthread_mutex_lock (&priv->lock);
        {
                ...
                priv->submit_log = 0;
                entry = __socket_ioq_new (this, &req->msg); //新建io请求队列, req拷贝进entry
                if (!entry)
                        goto unlock;
			...
			if (priv->own_thread) { //不执行
                ...
                if (!priv->own_thread && need_poll_out) { //poll out
                        /* first entry to wait. continue writing on POLLOUT */
                        priv->idx = event_select_on (ctx->event_pool, //注册事件，并且更改为poll_out模式
                                                     priv->sock,
                                                     priv->idx, -1, 1);
                }
        }
...
socket_init (rpc_transport_t *this)
{
	...
	int               session_id = 0;
        ...
        priv->ssl_enabled = _gf_false; //ssl关闭
	...
	priv->own_thread = priv->use_ssl; //关闭ssl，own_thread = 0
