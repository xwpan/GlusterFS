...
rpc_transport_load (glusterfs_ctx_t *ctx, dict_t *options, char *trans_name)
{
	...
	trans = GF_CALLOC (1, sizeof (struct rpc_transport), gf_common_mt_rpc_trans_t);//创建一个trans
        if (!trans)
                goto fail;
	...
	/* Backward compatibility */
        ret = dict_get_str (options, "transport-type", &type);  //获得transport type类型
	if (ret < 0) {
		ret = dict_set_str (options, "transport-type", "socket"); //默认设置成socket？
		if (ret < 0)
		...
		is_tcp = strcmp (type, "tcp"); //tcp类型
		is_unix = strcmp (type, "unix");  //unix类型
		is_ibsdp = strcmp (type, "ib-sdp"); //ib—sdp类型
		if ((is_tcp == 0) ||
		    (is_unix == 0) ||
		    (is_ibsdp == 0)) {
			if (is_unix == 0)
				ret = dict_set_str (options,
						    "transport.address-family",  //设置transport的协议族family为unix
						    "unix");
			if (is_ibsdp == 0)
				ret = dict_set_str (options,
						    "transport.address-family",//family为inet-sdp
						    "inet-sdp");
			...
			ret = dict_set_str (options,
					    "transport-type", "socket"); //   tcp/unix/ib-sdp 全部设置成socket类型
			if (ret < 0)
				gf_log ("dict", GF_LOG_DEBUG,
					"setting transport-type failed");
		}
	...
	ret = dict_get_str (options, "transport-type", &type); //获得最终的rpc_transport类型 （socket/rdma/dpdk）
	if (ret < 0) {
	...
	ret = gf_asprintf (&name, "%s/%s.so", RPC_TRANSPORTDIR, type);//在/usr/local/lib/3.9.3/rpc-transport下查找对应的动态库
        if (-1 == ret) {
                goto fail;
        }
	...
	handle = dlopen (name, RTLD_NOW|RTLD_GLOBAL);//handle可以获得动态库的符号
	if (handle == NULL) {
	...
	trans->ops = dlsym (handle, "tops");//赋值tops
	if (trans->ops == NULL) {
		gf_log ("rpc-transport", GF_LOG_ERROR,
	...
	*VOID(&(trans->init)) = dlsym (handle, "init");//赋值trand->init
	if (trans->init == NULL) {
		gf_log ("rpc-transport", GF_LOG_ERROR,
	...
	*VOID(&(trans->fini)) = dlsym (handle, "fini"); //赋值trans->fini
	if (trans->fini == NULL) {
		gf_log ("rpc-transport", GF_LOG_ERROR,
        ...
        *VOID(&(trans->reconfigure)) = dlsym (handle, "reconfigure");//赋值trans->reconfigure
	...
	ret = trans->init (trans); //启动trans->init（）函数
	if (ret != 0) {
		gf_log ("rpc-transport", GF_LOG_WARNING,
			"'%s' initialization failed", type);//log中输出：rdma初始化失败
		goto fail;
	}
...
rpc_transport_connect (rpc_transport_t *this, int port)
{
	...
	ret = this->ops->connect (this, port); //调用该xlator的ops中的连接方法rdma/socket
fail:
	return ret;
}
                                  ...
                                  int frame_timeout)
{
		...
		gf_log ("pxw", GF_LOG_WARNING, "UNIX connect fpath = %s", fpath);
        if (ret) {
                GF_FREE (fpath);
                goto out;
        }
